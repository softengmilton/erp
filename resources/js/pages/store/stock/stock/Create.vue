...
// Watchers
watch(
  [subtotal, () => form.shipping_cost, () => form.other_fees, totalQuantity],
  () => {
...
function distributeAdditionalCosts() {
  ...
  form.products.forEach(product => {
    const proportion = totalQuantity.value > 0 ? product.quantity / totalQuantity.value : 0;

    product.shipping_cost = shipping * proportion;
    product.other_fees = other * proportion;
    product.total_cost = (product.quantity * product.unit_cost) + product.shipping_cost + product.other_fees;
  });
}
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <div class="flex items-center justify-between mb-6 pb-4 border-b">
        <div>
          ...
          <p class="text-gray-600 mt-1">Fill in the details of your new stock inventory</p>
        </div>
        <img
      ...
      <div class="space-y-8">
        ...
        <div class="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              />
              <p v-if="form.errors.invoice_number" class="mt-1 text-sm text-red-600">
        ...
        <div class="bg-white p-6 rounded-lg border border-gray-200">
            ...
            <div class="relative">
              ...
              >
                ...
                >
                  ...
                  {{ product.name }}
                  <span class="ml-auto text-gray-500">${{ product.cost_price?.toFixed(2) || '0.00' }}</span>
                </li>
              </ul>
            </div>
          ...
          <div class="overflow-x-auto border border-gray-200 rounded-lg">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(item, index) in form.products" :key="item.id" class="hover:bg-gray-50">
          ...
          <div v-if="form.products.length === 0" class="text-center py-8 border border-gray-200 rounded-lg mt-4">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
            </svg>
            <h3 class="mt-2 text-sm font-medium text-gray-900">No products added</h3>
            <p class="mt-1 text-sm text-gray-500">Search and select products above to add them to this stock entry.</p>
          </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div>
              ...
              <div class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-md text-right font-medium text-gray-900">
                ${{ subtotal.toFixed(2) }}
              </div>
            </div>
            <div>
              ...
              <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                <div class="space-y-3">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium">${{ parseFloat(form.shipping_cost || 0).toFixed(2) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-sm text-gray-600">Other Fees:</span>
                    <span class="text-sm font-medium">${{ parseFloat(form.other_fees || 0).toFixed(2) }}</span>
                  </div>
                  <div class="border-t border-gray-200 pt-3 mt-2">
                    <div class="flex justify-between">
                      <span class="text-base font-semibold">Grand Total:</span>
                      <span class="text-base font-bold text-blue-600">${{ grandTotal.toFixed(2) }}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="bg-white p-6 rounded-lg border  border-gray-200">
          <div class="flex items-center mb-6 space-y-6">
            <div class="w-1 h-6 bg-blue-500 mr-3 rounded-full"></div>
            ...
            <div>
              ...
              <div class="rounded-lg border-2 border-dashed border-gray-300 p-6 text-center">
                <div v-if="imagePreview" class="space-y-4">
                  <img :src="imagePreview" alt="Document preview" class="w-full h-48 object-contain mx-auto rounded-md">
                  <button
                    @click="removeImage"
                    type="button"
                ...
                <div v-else class="space-y-3">
                  ...
                  <p class="text-sm text-gray-600">Upload supporting documents (PDF, JPG, PNG)</p>
 ...
 <!-- Submit Button -->
      <div class="flex justify-end pt-6 animate-on-load">
        <button
          ...
          style="background-color: #0984e3; box-shadow: 0 4px 0 #0767b1; transform: translateY(0);"
          :class="{'hover:bg-blue-700': !form.processing, 'active:transform active:translateY(1px) active:shadow-none': !form.processing}"
        >
