...
const breadcrumbs = [
  { title: 'Expenses', href: '/store/expenses' },
];
...
// function consoleLog(){
//   console.log(selectedExpense.value.name);
//   console.log(selectedExpense.value.amount);
//   console.log(selectedExpense.value.description);
//   console.log(selectedExpense.value.store_expense_type_id);
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6">
      ...
      <div class="overflow-x-auto border rounded-lg border-gray-200 bg-white">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Expense Name</th>
              <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Expense Type</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            class="hover:bg-gray-50 transition-colors duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ expense.store_expense_type.name }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ expense.amount }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ expense.description }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                <button 
                @click="openEditModal(expense)"
                class="text-blue-600 hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 rounded">Edit
              </button>
                <button 
                @click="openDeleteModal(expense)"
                class="text-red-600 hover:text-red-800 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-1 rounded">Delete
              </button>
              </td>
            </tr>
        ...
        <div class="relative bg-white/90 backdrop-blur-md rounded-lg p-6 w-full max-w-4xl shadow-xl border border-white/20 overflow-y-auto max-h-[90vh]">
          ...
          <div class="flex flex-col md:flex-row gap-6 mb-6">
            ...
            <div class="w-full md:w-2/3 space-y-4">
              ...
              <label class="block">
                <span class="block text-sm font-medium text-gray-700 mb-1">Expense Type</span>
                <select
          ...
          <div class="mb-6">
            <label class="block">
              <span class="block text-sm font-medium text-gray-700 mb-1">Description</span>
                <ckeditor :editor="editor" v-model="selectedExpense.description"
      ...
      </div> <!-- ✅ Entire modal wrapper ends here -->
